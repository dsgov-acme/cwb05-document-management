...
2. [view docs](http://api.cwb05.test/dm/swagger-ui/index.html)
...
```bash
curl -F file=@thumb.jpg api.cwb05.test/dm/docs
```
...
```bash
curl api.cwb05.test/dm/docs/40a9fe1d-aeeb-4d55-94b8-ca8c251f6b0e/status
```
...
```bash
curl api.cwb05.test/dm/docs/40a9fe1d-aeeb-4d55-94b8-ca8c251f6b0e/url
```
...
of [the local development script](https://github.com/dsgov-acme/cwb05-local-environment)
...
Those buckets are exposed for this and other services at the cluster internal URL `http://gcloud-storage-emulator-service.cwb05:4443/storage/v1/b`, which can be queried for existing buckets with a single `curl` call to that URL from any pod.
